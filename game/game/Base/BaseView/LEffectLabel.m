...
{
    //该方法只适合与动画全帧刷新
//    CADisplayLink *link = [CADisplayLink displayLinkWithTarget:self selector:@selector(performEffectAnimation)];
    [self performEffectAnimation];
    hnuSetWeakSelf;
    if (@available(iOS 10.0, *)) {
        NSTimer *timerTmp=[NSTimer scheduledTimerWithTimeInterval:3 repeats:true block:^(NSTimer *timer)
                           {
                               if (!weakSelf) {
                                   [timer invalidate];
                               }
                               else [weakSelf performEffectAnimation];
                           }];
        //滑动的时候需要切换mode
