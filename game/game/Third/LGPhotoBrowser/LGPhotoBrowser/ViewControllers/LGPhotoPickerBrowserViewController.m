...
- (UILabel *)pageLabel {
    if (!_pageLabel) {
        ...
        NSString * btm = isIphoneX?@"34":@"20";
        NSString *heightVfl = [NSString stringWithFormat:@"V:[pageLabel(ZLPickerPageCtrlH)]-%@-|",btm];
...
- (void)setupXGToolbar {
    if ((!_XGtoolbar)  && (!self.needHidenBar)) {
        CGFloat height = 44;
        CGFloat safe = isIphoneX?34:0;
        _XGtoolbar = [[LGPhotoPickerCustomToolBarView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.view.bounds) - height - safe, CGRectGetWidth(self.view.bounds), height) showType:self.showType];
