...
- (id)init {
    if (self = [super init]) {
        CGFloat staBar = [KKDataTool statusBarH];
        self.frame = CGRectMake(0, 0, SCREEN_WIDTH, 44 + staBar);
        ...
        [self.backBtn setFrame:CGRectMake(15, staBar, 44, 44)];
        [self.backBtn setImage:GetImage(@"btn_back_imagePicker.png") forState:UIControlStateNormal];
        [self addSubview:self.backBtn];
        self.selectBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        [self.selectBtn setFrame:CGRectMake(SCREEN_WIDTH - 40 - 15, staBar + 2, 40, 40)];
